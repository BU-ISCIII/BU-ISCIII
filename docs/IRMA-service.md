# How to perform viral services with IRMA
...
- `lablog`: **when running this script, make sure the refgenie micromamba environment has been activated!** This script, when run, generates several subscripts that should be launched sequentially. **They should be run by means of their corresponding `run` scripts, if that's the case**:
  ...
  - `_05_run_create_summary_stats.sh`: this final script creates summary statistics in parallel, launching one Slurm-array task per sample (maximum 50 concurrent tasks). Each task writes a temporary ``tmp_<sample>.tab`` file. Run ``_05_run_create_summary_stats.sh`` so that this array is submitted and then the temporary files are merged into ``summary_stats_YYYYMMDD.tab``, while removing these temporary files. **Do not launch this script until everything else is finished and do not use the final summary file until all jobs have completed successfully.**
