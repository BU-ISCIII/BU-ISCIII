...
Unlike `/data/ucct/bi`, `bioinfo_doc` is backed up but cannot be accesed from the HPC. This folder is for storing static files, like documentation, papers, posters, slides, ...
...
coding: documentation and templates to use in your own script
infrastructure: documentation about our computational resources
...
Whatever you do not know where it fits.
...
Here we keep track and statistics of all the services we make. They are classified by year and have the same names as their counterparts in `/data/ucct/bi/services_and_colaborations`. Logs from the `bioinfo_doc` module from the `buisciii-tools` are stored for each service year.
