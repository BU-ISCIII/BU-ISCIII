...
* `ANALYSIS01_SNIPPY`: a folder containg several subfolders whose scripts will be launched **sequentially** from now on:
  ...
  * `05-iqtree`: this folder contains a `lablog` file that does the following:
    ...
    * Creates a script called `_01_iqtreeall.sh` which will run iqtree on the `phylo.aln` file from the `04-snippy` folder, this time using the most appropriate model (**you'll have to manually revise the .log file from `_00_iqtreemfp.sh` in order to find out the best-fit model selected by ModelFinder, and then modify the `-m` option in the command**). Iqtree will be run with a specific number of CPU cores (by means of the `-T` option, 20 by default) and 1000 bootstrap replicates by default (by means of the `-B` option). The resulting files will be saved with the `phylo.iqtree.bootstrap` prefix (option `-pre`).
